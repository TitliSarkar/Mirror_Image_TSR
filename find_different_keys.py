...
'''
# find exclusive keys present in each subgroup (w/ and w/o freq)
def find_exclusive_keys(grp,protList, directory):
    exclusive_keys = []
    exclusive_keys_freq = []
    common_unq_keys = pickle.load(open(directory+"allProteins_common_unq_keys.p", "rb")) # common unique keys for all proteins
    
    f_out_unq = open(directory+str(grp)+"_exclusiveKey_Details.txt", "w")
    f_out_freq = open(directory+str(grp)+"_exclusiveKey_Details_freq.txt", "w")

    for p in protList:
        #print(p)
        df = pd.read_csv(directory+p+".triplets_29_35", sep="\t", usecols=[0],names=['key'])
        
        df_exclusive = df[~df['key'].isin(common_unq_keys)]#with freq
        
        exclusive_keys.extend(df_exclusive['key'].tolist())
