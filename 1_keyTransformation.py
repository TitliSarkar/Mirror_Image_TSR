...
parser = argparse.ArgumentParser(description='Key Calculation with chain info.')
...
def parallelcode(fileName): #fileName includes chain name
    ...
    outFile2 = open(data_dir+subdir+fileName+".keys_theta30_maxdist35", "w")
    fileTriplets = open(data_dir+subdir+fileName+".triplets_theta30_maxdist35","w")
    # Write Header
    ...
    for i in range(0,protLen-2):
        for j in range(i+1,protLen-1):
            for k in range(j+1, protLen):
                ...
                line = (str(key_2)+"\t"+str(aacd0)+"\t"+str(position0)+"\t"+str(aacd1)+"\t"+str(position1)+"\t"+str(aacd2)+"\t"+str(position2)+"\t"+str(classT1)+"\t"+str(Theta)+"\t"+str(classL1)+"\t"+str(maxDist)+"\t"+x0+"\t"+y0+"\t"+z0+"\t"+x1+"\t"+y1+"\t"+z1+"\t"+x2+"\t"+y2+"\t"+z2+"\t"+str(Theta1)+"\n")
